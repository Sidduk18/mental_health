...
export default function Exercises({ profile }: { profile: any }) {
      ...
      case 'breathing':
        return (
          <div className="py-12 flex flex-col items-center space-y-8">
            <motion.div 
              animate={{ 
                scale: currentStep === 0 ? 1.5 : currentStep === 2 ? 1 : 1.5,
                opacity: currentStep === 1 || currentStep === 3 ? 0.8 : 1
              }}
              transition={{ duration: 4, ease: "easeInOut" }}
              className="w-64 h-64 rounded-full bg-black flex items-center justify-center text-white"
            >
              <div className="text-center">
                <Wind className="w-12 h-12 mx-auto mb-2" />
                <p className="text-2xl font-black">{ex.steps[currentStep]}</p>
              </div>
            </motion.div>
            <button 
              onClick={() => setIsActive(!isActive)}
              className="bg-black text-white px-8 py-3 rounded-2xl font-bold flex items-center space-x-2"
            >
          ...
          <div className="grid grid-cols-1 gap-4 py-8">
            {ex.steps.map((step, i) => (
              <motion.div 
                ...
                className="p-6 bg-neutral-50 rounded-3xl border border-black/5 flex items-center space-x-4"
              >
      ...
      default:
        return (
          <div className="space-y-6 py-8">
            {ex.steps.map((step, i) => (
              <div key={i} className="p-6 bg-neutral-50 rounded-3xl border border-black/5 text-xl font-bold">
                {step}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="space-y-8 pb-20">
      <header>
        <h2 className="text-3xl font-black tracking-tight">Guided Exercises</h2>
        <p className="text-black/50">CBT, mindfulness, and breathing tools.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {exercises.map((ex) => (
          <motion.div 
            key={ex.id}
            whileHover={{ y: -5 }}
            className="bg-white p-8 rounded-[32px] border border-black/10 shadow-sm flex flex-col h-full group"
          >
            ...
            <p className="text-sm text-black/50 flex-1 mb-6">{ex.description}</p>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-widest opacity-40">{ex.duration}</span>
              <button 
                onClick={() => {
                ...
                className="bg-black text-white px-6 py-2 rounded-xl text-sm font-bold hover:scale-105 transition-transform"
              >
                Start
              </button>
            </div>
        ...
        {activeExercise && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white max-w-2xl w-full rounded-[48px] p-12 text-center space-y-8 relative max-h-[90vh] overflow-y-auto"
            >
              <button 
              ...
              <div className="space-y-2">
                ...
                <p className="text-black/50">Follow the steps below at your own pace.</p>
              </div>
              ...
              <button 
                onClick={() => setActiveExercise(null)}
                className="w-full bg-black text-white py-4 rounded-2xl font-bold text-xl hover:shadow-2xl transition-all"
              >
                Complete Session
              </button>
            </motion.div>
